...
public:
    ...
    state select_best_move(const board &b) const
    {
        ...
        for (state *move = after; move != after + 4; move++)
        {
            if (move->assign(b))
            {
                ...
                float est_value = 0;
                int num_empty = 0;
                std::vector<int> blank_id;
                board tmp = move->after_state();
                for(int index=0; index<16; index++){
                    if (!tmp.at(index)){
                        num_empty++;
                        blank_id.push_back(index);
                    }
                }
                for(int i=0; i<num_empty; i++){
                    board tmp2 = tmp;
                    board tmp4 = tmp;
                    tmp2.set(blank_id[i], 1);
                    tmp4.set(blank_id[i], 2);
                    
                    est_value += (1.0/num_empty)*0.9*estimate(tmp2) + (1.0/num_empty)*0.1*estimate(tmp4);
                    
                }

                move->set_value(move->reward() + est_value);
    ...
    void update_episode(std::vector<state> &path, float alpha = 0.1) const
    {
        ...
        /**
			 * TD backup step.
			 * Update strategy: V(S_t) = V(S_t) + alpha * (R_{t + 1} + alpha * V(S_{t + 1}) - V(S_t)))
			 */
			float board_state = 0; // Save for backup
			for (path.pop_back(); path.size(); path.pop_back()) {
				state &move = path.back(); // Get the latest state
				float error = move.reward() + board_state - estimate(move.before_state());
				board_state = update(move.before_state(), alpha * error);
			}
    }
     ...
     */
    void make_statistic(std::ofstream& file, size_t n, const board &b, int score, int unit = 1000)
    {
            ...
            if (n % 10000 ==0){
                ...
                for (int t = 1, c = 0; c < unit; c += stat[t++]){
                    if (stat[t] == 0)
                        continue;
                    int accu = std::accumulate(stat + t, stat + 16, 0);
                    file << "\t" << ((1 << t) & -2u) << "\t" << (accu * coef) << "%";
                    file << "\t(" << (stat[t] * coef) << "%)" << std::endl;
                }
            }
...
int main(int argc, const char *argv[])
{
    ...
    std::ofstream score_log_detail;
    score_log.open("log.txt");
    score_log_detail.open("log_detail.txt");
    ...
    tdl.add_feature(new pattern({0, 4, 5, 8, 10, 12}));
    tdl.add_feature(new pattern({4, 5, 6, 8, 9, 10}));
    tdl.add_feature(new pattern({0, 1, 2, 3, 4}));
    tdl.add_feature(new pattern({0, 1, 4, 5, 8}));
    ...
    //tdl.add_feature(new pattern({0, 1, 2, 3, 4, 8, 12}));
    //tdl.add_feature(new pattern({0, 4, 5, 8, 12, 13, 14}));
    ...
    for (size_t n = 1; n <= total; n++)
    {
        ...
        score_log << score << std::endl;

        // update by TD(0)
        tdl.update_episode(path, alpha);
        tdl.make_statistic(score_log_detail, n, b, score);
        path.clear();
        if(n % 10000 == 0){
